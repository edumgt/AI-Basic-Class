...
def run() -> dict:
    np.random.seed(42)
    result = {"chapter": "chapter03", "topic": "확률과 분포 시뮬레이션"}

    if "probability" == "numpy":
        arr = np.array([1, 2, 3, 4, 5])
        result["mean"] = float(arr.mean())
        result["std"] = float(arr.std())

    elif "probability" == "pandas":
    ...
    elif "probability" == "logistic_regression":
    ...
    elif "probability" == "decision_tree":
    ...
    elif "probability" == "random_forest":
    ...
    elif "probability" == "feature_engineering":
        df = pd.DataFrame({
            "age": [25, 30, 35, 40],
    ...
    elif "probability" == "regularization":
    ...
    elif "probability" == "dimensionality_reduction":
    ...
    elif "probability" == "outlier_detection":
    ...
    elif "probability" == "model_persistence":
