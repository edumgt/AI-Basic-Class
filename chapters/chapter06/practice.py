...
def run() -> dict:
    np.random.seed(42)
    result = {"chapter": "chapter06", "topic": "로지스틱 회귀"}

    if "logistic_regression" == "numpy":
        arr = np.array([1, 2, 3, 4, 5])
        result["mean"] = float(arr.mean())
        result["std"] = float(arr.std())

    elif "logistic_regression" == "pandas":
    ...
    elif "logistic_regression" == "probability":
        toss = np.random.binomial(1, 0.6, size=1000)
        result["estimated_p"] = float(toss.mean())

    elif "logistic_regression" == "linear_algebra":
        A = np.array([[2, 1], [1, 3]])
        b = np.array([1, 2])
        x = np.linalg.solve(A, b)
        result["solution"] = x.round(4).tolist()

    elif "logistic_regression" == "linear_regression":
    ...
    elif "logistic_regression" == "logistic_regression":
    ...
    elif "logistic_regression" == "decision_tree":
    ...
    elif "logistic_regression" == "kmeans":
    ...
    elif "logistic_regression" == "metrics":
    ...
    elif "logistic_regression" == "validation":
    ...
    elif "logistic_regression" == "feature_engineering":
        df = pd.DataFrame({
            "age": [25, 30, 35, 40],
    ...
    elif "logistic_regression" == "regularization":
    ...
    elif "logistic_regression" == "dimensionality_reduction":
    ...
    elif "logistic_regression" == "outlier_detection":
    ...
    elif "logistic_regression" == "time_series":
    ...
    elif "logistic_regression" == "pipeline":
        X, y = make_classification(n_samples=180, n_features=6, n_informative=4, random_state=42)
        pipe = Pipeline([
    ...
    elif "logistic_regression" == "model_persistence":
    ...
    elif "logistic_regression" == "serving":
        result["message"] = "FastAPI 백엔드(app/main.py)의 /api/chapters 엔드포인트를 확인하세요."

    elif "logistic_regression" == "mini_project":
