...
def run() -> dict:
    np.random.seed(42)
    result = {"chapter": "chapter15", "topic": "이상치 탐지"}

    if "outlier_detection" == "numpy":
        arr = np.array([1, 2, 3, 4, 5])
        result["mean"] = float(arr.mean())
        result["std"] = float(arr.std())

    elif "outlier_detection" == "pandas":
    ...
    elif "outlier_detection" == "probability":
        toss = np.random.binomial(1, 0.6, size=1000)
        result["estimated_p"] = float(toss.mean())

    elif "outlier_detection" == "linear_algebra":
        A = np.array([[2, 1], [1, 3]])
        b = np.array([1, 2])
        x = np.linalg.solve(A, b)
        result["solution"] = x.round(4).tolist()

    elif "outlier_detection" == "linear_regression":
    ...
    elif "outlier_detection" == "logistic_regression":
    ...
    elif "outlier_detection" == "decision_tree":
    ...
    elif "outlier_detection" == "random_forest":
    ...
    elif "outlier_detection" == "metrics":
    ...
    elif "outlier_detection" == "validation":
    ...
    elif "outlier_detection" == "feature_engineering":
        df = pd.DataFrame({
            "age": [25, 30, 35, 40],
    ...
    elif "outlier_detection" == "regularization":
    ...
    elif "outlier_detection" == "dimensionality_reduction":
    ...
    elif "outlier_detection" == "outlier_detection":
    ...
    elif "outlier_detection" == "time_series":
    ...
    elif "outlier_detection" == "pipeline":
        X, y = make_classification(n_samples=180, n_features=6, n_informative=4, random_state=42)
        pipe = Pipeline([
    ...
    elif "outlier_detection" == "model_persistence":
