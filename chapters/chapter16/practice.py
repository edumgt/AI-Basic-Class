...
def run() -> dict:
    np.random.seed(42)
    result = {"chapter": "chapter16", "topic": "시계열 기초"}

    if "time_series" == "numpy":
        arr = np.array([1, 2, 3, 4, 5])
        result["mean"] = float(arr.mean())
        result["std"] = float(arr.std())

    elif "time_series" == "pandas":
    ...
    elif "time_series" == "logistic_regression":
    ...
    elif "time_series" == "decision_tree":
    ...
    elif "time_series" == "random_forest":
    ...
    elif "time_series" == "feature_engineering":
        df = pd.DataFrame({
            "age": [25, 30, 35, 40],
    ...
    elif "time_series" == "regularization":
    ...
    elif "time_series" == "dimensionality_reduction":
    ...
    elif "time_series" == "outlier_detection":
    ...
    elif "time_series" == "model_persistence":
